...
export default class App extends React.Component {
    ...
    render() {
        if (this.state.loadFilms) {
            ...
            return (
                <>
                    <button onClick={() => this.setState({ loadFilms: true })} className="btn btn-secondary btn-sm m-3">Load Films</button>
                    <button onClick={() => this.setState({ loadPeople: true })} className="btn btn-secondary btn-sm m-3">Load People</button>
                </>
            )
        }
    }
